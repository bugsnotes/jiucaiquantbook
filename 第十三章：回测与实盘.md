# 第十三章：回测与实盘 🎯

> 💡 **本章导读**：回测是策略从理论走向实践的桥梁。然而,很多看似完美的回测结果在实盘中却惨遭滑铁卢。本章将深入讲解回测的正确方法、常见陷阱、如何识别真正优秀的策略,以及如何平稳地从回测过渡到实盘交易。

---

## 12.1 回测基础知识 📚

### 12.1.1 什么是回测？

**回测(Backtesting)**：使用历史数据模拟策略的交易过程,评估策略的历史表现。

**回测的目的**：
1. 验证策略逻辑是否正确
2. 评估策略的盈利能力
3. 了解策略的风险特征
4. 优化策略参数
5. 增强交易信心

### 12.1.2 回测的基本框架

```python
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt

class SimpleBacktestEngine:
    """简单回测引擎"""

    def __init__(self, initial_capital, commission_rate=0.0003):
        """
        参数:
        initial_capital: float, 初始资金
        commission_rate: float, 手续费率
        """
        self.initial_capital = initial_capital
        self.commission_rate = commission_rate
        self.reset()

    def reset(self):
        """重置回测状态"""
        self.cash = self.initial_capital
        self.position = 0  # 持仓数量
        self.trades = []   # 交易记录
        self.equity_curve = []  # 资金曲线

    def execute_trade(self, date, price, signal):
        """
        执行交易

        参数:
        date: 日期
        price: float, 当前价格
        signal: int, 交易信号(1: 买入, -1: 卖出, 0: 持有)

        返回:
        dict: 交易信息
        """
        trade_info = None

        if signal == 1 and self.position == 0:
            # 买入信号且当前空仓
            # 计算可买入数量(扣除手续费)
            available_cash = self.cash * (1 - self.commission_rate)
            shares = int(available_cash / price)

            if shares > 0:
                cost = shares * price
                commission = cost * self.commission_rate
                self.cash -= (cost + commission)
                self.position = shares

                trade_info = {
                    'date': date,
                    'type': 'BUY',
                    'price': price,
                    'shares': shares,
                    'cost': cost,
                    'commission': commission
                }
                self.trades.append(trade_info)

        elif signal == -1 and self.position > 0:
            # 卖出信号且当前持仓
            revenue = self.position * price
            commission = revenue * self.commission_rate
            self.cash += (revenue - commission)

            trade_info = {
                'date': date,
                'type': 'SELL',
                'price': price,
                'shares': self.position,
                'revenue': revenue,
                'commission': commission
            }
            self.trades.append(trade_info)
            self.position = 0

        # 记录资金曲线
        portfolio_value = self.cash + self.position * price
        self.equity_curve.append({
            'date': date,
            'cash': self.cash,
            'position': self.position,
            'price': price,
            'portfolio_value': portfolio_value
        })

        return trade_info

    def run_backtest(self, data, signal_column='signal'):
        """
        运行回测

        参数:
        data: DataFrame, 包含价格和信号的数据
        signal_column: str, 信号列名

        返回:
        dict: 回测结果
        """
        self.reset()

        for idx, row in data.iterrows():
            self.execute_trade(
                date=idx,
                price=row['close'],
                signal=row[signal_column]
            )

        # 如果最后还持有仓位,按最后价格平仓
        if self.position > 0:
            last_date = data.index[-1]
            last_price = data.iloc[-1]['close']
            self.execute_trade(last_date, last_price, signal=-1)

        return self.calculate_metrics()

    def calculate_metrics(self):
        """
        计算回测指标

        返回:
        dict: 各项指标
        """
        equity_df = pd.DataFrame(self.equity_curve)
        equity_df.set_index('date', inplace=True)

        final_value = equity_df['portfolio_value'].iloc[-1]
        total_return = (final_value - self.initial_capital) / self.initial_capital

        # 计算日收益率
        equity_df['returns'] = equity_df['portfolio_value'].pct_change()

        # 年化收益率
        trading_days = len(equity_df)
        annual_return = (1 + total_return) ** (252 / trading_days) - 1

        # 年化波动率
        annual_volatility = equity_df['returns'].std() * np.sqrt(252)

        # 夏普比率
        sharpe_ratio = annual_return / annual_volatility if annual_volatility > 0 else 0

        # 最大回撤
        cumulative_max = equity_df['portfolio_value'].cummax()
        drawdown = (equity_df['portfolio_value'] - cumulative_max) / cumulative_max
        max_drawdown = drawdown.min()

        # 交易统计
        trades_df = pd.DataFrame(self.trades)
        num_trades = len(trades_df) // 2  # 买卖成对

        # 计算每笔交易的盈亏
        if num_trades > 0:
            buy_trades = trades_df[trades_df['type'] == 'BUY'].reset_index(drop=True)
            sell_trades = trades_df[trades_df['type'] == 'SELL'].reset_index(drop=True)

            trade_returns = []
            for i in range(min(len(buy_trades), len(sell_trades))):
                buy_price = buy_trades.iloc[i]['price']
                sell_price = sell_trades.iloc[i]['price']
                ret = (sell_price - buy_price) / buy_price
                trade_returns.append(ret)

            win_trades = [r for r in trade_returns if r > 0]
            lose_trades = [r for r in trade_returns if r < 0]

            win_rate = len(win_trades) / len(trade_returns) if trade_returns else 0
            avg_win = np.mean(win_trades) if win_trades else 0
            avg_loss = np.mean(lose_trades) if lose_trades else 0
            profit_loss_ratio = abs(avg_win / avg_loss) if avg_loss != 0 else 0
        else:
            win_rate = 0
            avg_win = 0
            avg_loss = 0
            profit_loss_ratio = 0

        return {
            'initial_capital': self.initial_capital,
            'final_value': final_value,
            'total_return': total_return,
            'annual_return': annual_return,
            'annual_volatility': annual_volatility,
            'sharpe_ratio': sharpe_ratio,
            'max_drawdown': max_drawdown,
            'num_trades': num_trades,
            'win_rate': win_rate,
            'avg_win': avg_win,
            'avg_loss': avg_loss,
            'profit_loss_ratio': profit_loss_ratio,
            'equity_curve': equity_df
        }

    def plot_results(self, results):
        """
        可视化回测结果

        参数:
        results: dict, 回测结果
        """
        equity_df = results['equity_curve']

        fig, axes = plt.subplots(3, 1, figsize=(14, 10))

        # 资金曲线
        axes[0].plot(equity_df.index, equity_df['portfolio_value'], label='策略净值')
        axes[0].axhline(y=self.initial_capital, color='r',
                       linestyle='--', alpha=0.5, label='初始资金')
        axes[0].set_ylabel('资金(元)')
        axes[0].set_title(f'回测结果 | 总收益: {results["total_return"]:.2%} | '
                         f'夏普: {results["sharpe_ratio"]:.2f} | '
                         f'最大回撤: {results["max_drawdown"]:.2%}')
        axes[0].legend()
        axes[0].grid(True, alpha=0.3)

        # 回撤曲线
        cumulative_max = equity_df['portfolio_value'].cummax()
        drawdown = (equity_df['portfolio_value'] - cumulative_max) / cumulative_max
        axes[1].fill_between(equity_df.index, drawdown, 0,
                            alpha=0.3, color='red', label='回撤')
        axes[1].set_ylabel('回撤比例')
        axes[1].set_title('回撤曲线')
        axes[1].legend()
        axes[1].grid(True, alpha=0.3)

        # 日收益率分布
        axes[2].hist(equity_df['returns'].dropna(), bins=50, alpha=0.7, edgecolor='black')
        axes[2].set_xlabel('日收益率')
        axes[2].set_ylabel('频数')
        axes[2].set_title('日收益率分布')
        axes[2].grid(True, alpha=0.3)

        plt.tight_layout()
        # plt.savefig('backtest_results.png', dpi=300, bbox_inches='tight')
        print("\n回测可视化图表已生成")

# 示例使用
# 生成模拟数据
np.random.seed(42)
dates = pd.date_range('2023-01-01', periods=252, freq='D')
prices = 100 + np.cumsum(np.random.randn(252) * 2)
data = pd.DataFrame({
    'close': prices
}, index=dates)

# 简单的双均线策略信号
data['MA5'] = data['close'].rolling(window=5).mean()
data['MA20'] = data['close'].rolling(window=20).mean()
data['signal'] = 0
data.loc[data['MA5'] > data['MA20'], 'signal'] = 1
data.loc[data['MA5'] < data['MA20'], 'signal'] = -1

# 运行回测
engine = SimpleBacktestEngine(initial_capital=100000, commission_rate=0.0003)
results = engine.run_backtest(data)

# 打印结果
print("=" * 60)
print("回测结果摘要")
print("=" * 60)
print(f"初始资金: {results['initial_capital']:,.0f}元")
print(f"最终资金: {results['final_value']:,.0f}元")
print(f"总收益率: {results['total_return']:.2%}")
print(f"年化收益率: {results['annual_return']:.2%}")
print(f"年化波动率: {results['annual_volatility']:.2%}")
print(f"夏普比率: {results['sharpe_ratio']:.2f}")
print(f"最大回撤: {results['max_drawdown']:.2%}")
print(f"交易次数: {results['num_trades']}")
print(f"胜率: {results['win_rate']:.2%}")
print(f"盈亏比: {results['profit_loss_ratio']:.2f}")
print("=" * 60)
```

### 12.1.3 回测数据的要求

**数据质量直接影响回测可靠性！**

| 要求 | 说明 | 重要性 |
|-----|------|-------|
| **完整性** | 无缺失数据,或合理填充 | ⭐⭐⭐⭐⭐ |
| **准确性** | 数据无错误(如价格异常) | ⭐⭐⭐⭐⭐ |
| **时效性** | 数据及时更新 | ⭐⭐⭐⭐ |
| **一致性** | 格式统一,时区正确 | ⭐⭐⭐⭐ |
| **足够的历史长度** | 至少3-5年数据 | ⭐⭐⭐⭐ |
| **包含分红配股** | 复权处理 | ⭐⭐⭐⭐ |
| **高频数据质量** | Tick数据的准确性 | ⭐⭐⭐⭐⭐ (高频策略) |

```python
def check_data_quality(data):
    """
    检查数据质量

    参数:
    data: DataFrame, 价格数据

    返回:
    dict: 数据质量报告
    """
    report = {}

    # 1. 缺失值检查
    missing_count = data.isnull().sum().sum()
    missing_pct = missing_count / (len(data) * len(data.columns))
    report['missing_values'] = {
        'count': missing_count,
        'percentage': missing_pct
    }

    # 2. 异常值检查(价格突变)
    if 'close' in data.columns:
        returns = data['close'].pct_change()
        # 定义异常为超过3倍标准差
        threshold = 3 * returns.std()
        outliers = returns[abs(returns) > threshold]
        report['outliers'] = {
            'count': len(outliers),
            'dates': outliers.index.tolist()
        }

    # 3. 数据连续性检查
    if isinstance(data.index, pd.DatetimeIndex):
        # 检查日期间隔
        date_diffs = data.index.to_series().diff()
        # 假设应该是日线数据,工作日间隔不应超过3天
        gaps = date_diffs[date_diffs > pd.Timedelta(days=3)]
        report['date_gaps'] = {
            'count': len(gaps),
            'gaps': gaps.index.tolist()
        }

    # 4. 数据长度
    report['data_length'] = {
        'total_days': len(data),
        'years': len(data) / 252  # 假设每年252个交易日
    }

    return report

# 示例
quality_report = check_data_quality(data)
print("\n数据质量报告:")
print(f"缺失值: {quality_report['missing_values']['count']}个 "
      f"({quality_report['missing_values']['percentage']:.2%})")
print(f"异常值: {quality_report['outliers']['count']}个")
print(f"数据长度: {quality_report['data_length']['total_days']}天 "
      f"(约{quality_report['data_length']['years']:.1f}年)")
```

---

## 12.2 回测常见陷阱 ⚠️

回测中有很多陷阱会导致结果过于乐观,实盘时大失所望。

### 12.2.1 前视偏差 (Look-Ahead Bias)

**定义**：在回测中使用了当时还不可获得的未来信息。

**常见情况**：
1. 使用未来数据计算信号
2. 使用当日收盘价作为买入价
3. 财务数据发布日期处理不当

```python
# ❌ 错误示例:前视偏差
def wrong_strategy(data):
    """使用了未来信息"""
    data['signal'] = 0

    for i in range(len(data) - 1):
        # 错误!这里使用了第i+1天(未来)的信息来决定第i天的交易
        if data.iloc[i+1]['close'] > data.iloc[i]['close']:
            data.iloc[i, data.columns.get_loc('signal')] = 1

    return data

# ✅ 正确示例:避免前视偏差
def correct_strategy(data):
    """只使用历史信息"""
    data = data.copy()
    data['signal'] = 0

    for i in range(1, len(data)):
        # 正确!使用第i-1天及之前的信息来决定第i天的交易
        if data.iloc[i-1]['close'] > data.iloc[i-2]['close']:
            data.iloc[i, data.columns.get_loc('signal')] = 1

    return data

# 收盘价交易陷阱
print("⚠️ 收盘价交易陷阱:")
print("错误: 看到收盘价100元,发出买入信号,按100元成交")
print("现实: 看到收盘价时,交易已结束,只能第二天开盘买入")
print("\n正确做法:")
print("1. 使用第二天的开盘价成交")
print("2. 或者在盘中使用实时价格(高频策略)")
```

### 12.2.2 幸存者偏差 (Survivorship Bias)

**定义**：只使用了当前仍然存在的标的数据,忽略了已退市/摘牌的标的。

```python
class SurvivorshipBiasExample:
    """幸存者偏差示例"""

    @staticmethod
    def demonstrate_bias():
        """演示幸存者偏差的影响"""

        # 假设有10只股票
        stocks = [
            {'name': 'A', 'return': 0.5, 'survived': True},   # 幸存,涨50%
            {'name': 'B', 'return': 0.3, 'survived': True},   # 幸存,涨30%
            {'name': 'C', 'return': -0.8, 'survived': False}, # 退市,跌80%
            {'name': 'D', 'return': 0.2, 'survived': True},   # 幸存,涨20%
            {'name': 'E', 'return': -0.9, 'survived': False}, # 退市,跌90%
            {'name': 'F', 'return': 0.4, 'survived': True},   # 幸存,涨40%
            {'name': 'G', 'return': -0.7, 'survived': False}, # 退市,跌70%
            {'name': 'H', 'return': 0.1, 'survived': True},   # 幸存,涨10%
            {'name': 'I', 'return': 0.25, 'survived': True},  # 幸存,涨25%
            {'name': 'J', 'return': -0.6, 'survived': False}, # 退市,跌60%
        ]

        # 只看幸存股票(错误)
        survived_stocks = [s for s in stocks if s['survived']]
        biased_return = np.mean([s['return'] for s in survived_stocks])

        # 看所有股票(正确)
        true_return = np.mean([s['return'] for s in stocks])

        print("幸存者偏差示例:")
        print(f"只看幸存股票的平均收益: {biased_return:.2%}")
        print(f"包含所有股票的真实收益: {true_return:.2%}")
        print(f"偏差: {biased_return - true_return:.2%}")

        return biased_return, true_return

SurvivorshipBiasExample.demonstrate_bias()
```

**如何避免**：
- 使用包含退市股票的完整数据库
- 在策略中模拟退市处理(股价归零)
- 谨慎对待历史数据提供商的"幸存股票池"

### 12.2.3 过度拟合 (Overfitting)

**定义**：策略过度适应历史数据的噪音,而非捕捉真实规律。

```python
import matplotlib.pyplot as plt
from sklearn.preprocessing import PolynomialFeatures
from sklearn.linear_model import LinearRegression
from sklearn.pipeline import Pipeline

def demonstrate_overfitting():
    """演示过度拟合"""

    np.random.seed(42)

    # 生成真实关系: y = sin(x) + noise
    X = np.linspace(0, 10, 50)
    y_true = np.sin(X)
    y = y_true + np.random.randn(50) * 0.3

    X_plot = np.linspace(0, 10, 200)
    y_plot_true = np.sin(X_plot)

    fig, axes = plt.subplots(1, 3, figsize=(15, 4))

    # 简单模型(欠拟合)
    model1 = Pipeline([
        ('poly', PolynomialFeatures(degree=1)),
        ('linear', LinearRegression())
    ])
    model1.fit(X.reshape(-1, 1), y)
    y_pred1 = model1.predict(X_plot.reshape(-1, 1))

    axes[0].scatter(X, y, alpha=0.5, label='数据点')
    axes[0].plot(X_plot, y_plot_true, 'g--', label='真实关系', linewidth=2)
    axes[0].plot(X_plot, y_pred1, 'r-', label='模型拟合', linewidth=2)
    axes[0].set_title('欠拟合(过于简单)')
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)

    # 合适的模型
    model2 = Pipeline([
        ('poly', PolynomialFeatures(degree=3)),
        ('linear', LinearRegression())
    ])
    model2.fit(X.reshape(-1, 1), y)
    y_pred2 = model2.predict(X_plot.reshape(-1, 1))

    axes[1].scatter(X, y, alpha=0.5, label='数据点')
    axes[1].plot(X_plot, y_plot_true, 'g--', label='真实关系', linewidth=2)
    axes[1].plot(X_plot, y_pred2, 'r-', label='模型拟合', linewidth=2)
    axes[1].set_title('合适的拟合')
    axes[1].legend()
    axes[1].grid(True, alpha=0.3)

    # 过拟合模型
    model3 = Pipeline([
        ('poly', PolynomialFeatures(degree=15)),
        ('linear', LinearRegression())
    ])
    model3.fit(X.reshape(-1, 1), y)
    y_pred3 = model3.predict(X_plot.reshape(-1, 1))

    axes[2].scatter(X, y, alpha=0.5, label='数据点')
    axes[2].plot(X_plot, y_plot_true, 'g--', label='真实关系', linewidth=2)
    axes[2].plot(X_plot, y_pred3, 'r-', label='模型拟合', linewidth=2)
    axes[2].set_title('过拟合(过于复杂)')
    axes[2].legend()
    axes[2].grid(True, alpha=0.3)

    plt.tight_layout()
    # plt.savefig('overfitting_demo.png', dpi=300, bbox_inches='tight')
    print("过拟合演示图表已生成")

# demonstrate_overfitting()

print("\n如何避免过度拟合:")
print("1. 使用样本外验证(训练集/测试集分离)")
print("2. 交叉验证")
print("3. 保持策略简单")
print("4. 限制参数数量")
print("5. 使用正则化")
print("6. 注意参数优化过程")
```

### 12.2.4 交易成本忽视

**很多回测结果在扣除真实交易成本后,收益大幅降低甚至为负。**

```python
class TradingCostAnalysis:
    """交易成本分析"""

    def __init__(self):
        pass

    @staticmethod
    def calculate_total_costs(trades, price, amount):
        """
        计算总交易成本

        参数:
        trades: int, 交易次数
        price: float, 平均价格
        amount: float, 平均交易金额

        返回:
        dict: 各项成本
        """
        # 中国A股典型成本(买卖一次的总成本)
        commission = amount * 0.0003  # 券商佣金0.03%,双向
        stamp_tax = amount * 0.001    # 印花税0.1%,仅卖出
        transfer_fee = amount * 0.00002  # 过户费0.002%,双向

        single_trade_cost = (commission * 2) + stamp_tax + (transfer_fee * 2)
        total_cost = single_trade_cost * trades

        return {
            'commission': commission * 2 * trades,
            'stamp_tax': stamp_tax * trades,
            'transfer_fee': transfer_fee * 2 * trades,
            'single_trade_cost': single_trade_cost,
            'total_cost': total_cost,
            'cost_rate': single_trade_cost / amount
        }

    @staticmethod
    def compare_with_without_costs():
        """对比有无交易成本的回测结果"""

        # 假设策略
        initial_capital = 100000
        num_trades = 100  # 一年100次交易
        avg_trade_amount = 20000
        gross_return = 0.20  # 毛收益率20%

        costs = TradingCostAnalysis.calculate_total_costs(
            trades=num_trades,
            price=50,
            amount=avg_trade_amount
        )

        net_return = gross_return - (costs['total_cost'] / initial_capital)

        print("交易成本影响分析:")
        print("=" * 50)
        print(f"初始资金: {initial_capital:,.0f}元")
        print(f"年交易次数: {num_trades}")
        print(f"平均每笔交易金额: {avg_trade_amount:,.0f}元")
        print(f"\n单次买卖成本: {costs['single_trade_cost']:.2f}元 "
              f"({costs['cost_rate']:.2%})")
        print(f"  - 佣金: {costs['commission'] / num_trades:.2f}元")
        print(f"  - 印花税: {costs['stamp_tax'] / num_trades:.2f}元")
        print(f"  - 过户费: {costs['transfer_fee'] / num_trades:.2f}元")
        print(f"\n年度总成本: {costs['total_cost']:,.2f}元")
        print(f"\n毛收益率: {gross_return:.2%}")
        print(f"净收益率: {net_return:.2%}")
        print(f"成本影响: {gross_return - net_return:.2%}")
        print("=" * 50)

TradingCostAnalysis.compare_with_without_costs()
```

**交易成本清单**：
- ✅ 佣金
- ✅ 印花税(中国A股)
- ✅ 过户费
- ✅ 滑点(市价单与实际成交价差异)
- ✅ 市场冲击成本(大单对价格的影响)
- ✅ 融资融券利息(杠杆交易)

### 12.2.5 流动性假设

**很多回测假设可以按照预期价格立即成交,但实际中可能面临流动性不足。**

```python
def check_liquidity_risk(data, position_value, max_volume_pct=0.05):
    """
    检查流动性风险

    参数:
    data: DataFrame, 包含成交量数据
    position_value: float, 拟建仓位价值
    max_volume_pct: float, 不应超过日均成交量的百分比

    返回:
    dict: 流动性分析
    """
    if 'volume' not in data.columns or 'close' not in data.columns:
        return {'error': 'Data must contain volume and close columns'}

    # 计算日均成交额
    data['turnover'] = data['volume'] * data['close']
    avg_daily_turnover = data['turnover'].mean()

    # 建议的最大仓位
    max_position = avg_daily_turnover * max_volume_pct

    # 评估
    is_liquid_enough = position_value <= max_position
    liquidity_ratio = position_value / max_position

    return {
        'avg_daily_turnover': avg_daily_turnover,
        'position_value': position_value,
        'max_safe_position': max_position,
        'is_liquid_enough': is_liquid_enough,
        'liquidity_ratio': liquidity_ratio,
        'warning': '流动性不足,可能面临滑点或无法成交' if not is_liquid_enough else '流动性充足'
    }

# 示例
# 假设数据包含成交量
data_with_volume = data.copy()
data_with_volume['volume'] = np.random.randint(1000000, 5000000, len(data))

position_value = 500000  # 想要建仓50万
liquidity_analysis = check_liquidity_risk(data_with_volume, position_value)

print("\n流动性分析:")
print(f"日均成交额: {liquidity_analysis['avg_daily_turnover']:,.0f}元")
print(f"拟建仓位: {liquidity_analysis['position_value']:,.0f}元")
print(f"安全仓位上限: {liquidity_analysis['max_safe_position']:,.0f}元")
print(f"流动性比率: {liquidity_analysis['liquidity_ratio']:.2f}")
print(f"评估: {liquidity_analysis['warning']}")
```

---

## 12.3 如何识别好策略 ✨

### 12.3.1 样本外验证

**将数据分为训练集和测试集,在训练集上开发策略,在测试集上验证。**

```python
class WalkForwardAnalysis:
    """滚动优化分析(Walk-Forward Analysis)"""

    def __init__(self, train_period, test_period):
        """
        参数:
        train_period: int, 训练期长度(天)
        test_period: int, 测试期长度(天)
        """
        self.train_period = train_period
        self.test_period = test_period

    def split_data(self, data):
        """
        滚动分割数据

        参数:
        data: DataFrame, 完整数据

        返回:
        list: [(训练集, 测试集), ...]
        """
        splits = []
        total_length = len(data)
        step = self.test_period

        start = 0
        while start + self.train_period + self.test_period <= total_length:
            train_end = start + self.train_period
            test_end = train_end + self.test_period

            train_data = data.iloc[start:train_end]
            test_data = data.iloc[train_end:test_end]

            splits.append((train_data, test_data))

            start += step

        return splits

    def analyze(self, data, strategy_func):
        """
        执行滚动分析

        参数:
        data: DataFrame, 完整数据
        strategy_func: function, 策略函数,接受数据和参数,返回信号

        返回:
        dict: 分析结果
        """
        splits = self.split_data(data)

        train_results = []
        test_results = []

        for i, (train_data, test_data) in enumerate(splits):
            # 在训练集上优化参数(这里简化,实际应该进行参数搜索)
            train_signals = strategy_func(train_data)
            train_returns = self.calculate_returns(train_data, train_signals)
            train_results.append(train_returns)

            # 在测试集上验证
            test_signals = strategy_func(test_data)
            test_returns = self.calculate_returns(test_data, test_signals)
            test_results.append(test_returns)

        return {
            'num_splits': len(splits),
            'train_avg_return': np.mean(train_results),
            'test_avg_return': np.mean(test_results),
            'train_std': np.std(train_results),
            'test_std': np.std(test_results),
            'degradation': np.mean(train_results) - np.mean(test_results),
            'consistency': np.corrcoef(train_results, test_results)[0, 1]
        }

    @staticmethod
    def calculate_returns(data, signals):
        """简单计算收益率"""
        returns = data['close'].pct_change()
        strategy_returns = (signals.shift(1) * returns).sum()
        return strategy_returns

# 示例
def simple_strategy(data):
    """简单的双均线策略"""
    signals = pd.Series(0, index=data.index)
    ma5 = data['close'].rolling(5).mean()
    ma20 = data['close'].rolling(20).mean()
    signals[ma5 > ma20] = 1
    signals[ma5 < ma20] = -1
    return signals

wfa = WalkForwardAnalysis(train_period=120, test_period=60)
results = wfa.analyze(data, simple_strategy)

print("\n滚动优化分析结果:")
print(f"分割次数: {results['num_splits']}")
print(f"训练集平均收益: {results['train_avg_return']:.2%}")
print(f"测试集平均收益: {results['test_avg_return']:.2%}")
print(f"性能衰减: {results['degradation']:.2%}")
print(f"一致性: {results['consistency']:.2f}")

if results['degradation'] > 0.05:
    print("⚠️ 警告: 样本外表现显著差于样本内,可能存在过拟合")
```

### 12.3.2 关键指标评估

```python
class StrategyEvaluator:
    """策略综合评估"""

    def __init__(self):
        self.criteria = {
            'min_sharpe': 1.0,
            'max_drawdown': -0.20,
            'min_win_rate': 0.40,
            'min_profit_loss_ratio': 1.2,
            'min_num_trades': 30
        }

    def evaluate(self, backtest_results):
        """
        综合评估策略

        参数:
        backtest_results: dict, 回测结果

        返回:
        dict: 评估结果和建议
        """
        scores = {}
        passed = True

        # 1. 夏普比率
        sharpe = backtest_results['sharpe_ratio']
        scores['sharpe'] = {
            'value': sharpe,
            'criterion': self.criteria['min_sharpe'],
            'passed': sharpe >= self.criteria['min_sharpe'],
            'grade': self._grade_sharpe(sharpe)
        }
        if not scores['sharpe']['passed']:
            passed = False

        # 2. 最大回撤
        max_dd = backtest_results['max_drawdown']
        scores['max_drawdown'] = {
            'value': max_dd,
            'criterion': self.criteria['max_drawdown'],
            'passed': max_dd >= self.criteria['max_drawdown'],
            'grade': self._grade_drawdown(max_dd)
        }
        if not scores['max_drawdown']['passed']:
            passed = False

        # 3. 胜率
        win_rate = backtest_results['win_rate']
        scores['win_rate'] = {
            'value': win_rate,
            'criterion': self.criteria['min_win_rate'],
            'passed': win_rate >= self.criteria['min_win_rate'],
            'grade': self._grade_win_rate(win_rate)
        }
        if not scores['win_rate']['passed']:
            passed = False

        # 4. 盈亏比
        pl_ratio = backtest_results['profit_loss_ratio']
        scores['profit_loss_ratio'] = {
            'value': pl_ratio,
            'criterion': self.criteria['min_profit_loss_ratio'],
            'passed': pl_ratio >= self.criteria['min_profit_loss_ratio'],
            'grade': self._grade_pl_ratio(pl_ratio)
        }
        if not scores['profit_loss_ratio']['passed']:
            passed = False

        # 5. 交易次数
        num_trades = backtest_results['num_trades']
        scores['num_trades'] = {
            'value': num_trades,
            'criterion': self.criteria['min_num_trades'],
            'passed': num_trades >= self.criteria['min_num_trades'],
            'grade': 'A' if num_trades >= 50 else ('B' if num_trades >= 30 else 'C')
        }
        if not scores['num_trades']['passed']:
            passed = False

        # 综合评分
        overall_grade = self._calculate_overall_grade(scores)

        return {
            'scores': scores,
            'passed': passed,
            'overall_grade': overall_grade,
            'recommendation': self._get_recommendation(scores, overall_grade)
        }

    @staticmethod
    def _grade_sharpe(sharpe):
        if sharpe >= 2.0:
            return 'A'
        elif sharpe >= 1.5:
            return 'B'
        elif sharpe >= 1.0:
            return 'C'
        else:
            return 'D'

    @staticmethod
    def _grade_drawdown(dd):
        if dd >= -0.10:
            return 'A'
        elif dd >= -0.15:
            return 'B'
        elif dd >= -0.20:
            return 'C'
        else:
            return 'D'

    @staticmethod
    def _grade_win_rate(wr):
        if wr >= 0.60:
            return 'A'
        elif wr >= 0.50:
            return 'B'
        elif wr >= 0.40:
            return 'C'
        else:
            return 'D'

    @staticmethod
    def _grade_pl_ratio(plr):
        if plr >= 2.0:
            return 'A'
        elif plr >= 1.5:
            return 'B'
        elif plr >= 1.2:
            return 'C'
        else:
            return 'D'

    @staticmethod
    def _calculate_overall_grade(scores):
        grades = [s['grade'] for s in scores.values()]
        grade_values = {'A': 4, 'B': 3, 'C': 2, 'D': 1}
        avg = np.mean([grade_values[g] for g in grades])

        if avg >= 3.5:
            return 'A'
        elif avg >= 2.5:
            return 'B'
        elif avg >= 1.5:
            return 'C'
        else:
            return 'D'

    @staticmethod
    def _get_recommendation(scores, overall_grade):
        if overall_grade == 'A':
            return "优秀策略,可以考虑小资金实盘测试"
        elif overall_grade == 'B':
            return "良好策略,建议进一步优化后实盘"
        elif overall_grade == 'C':
            return "一般策略,需要显著改进"
        else:
            return "不建议实盘,需要重新设计"

# 示例
evaluator = StrategyEvaluator()
evaluation = evaluator.evaluate(results)

print("\n策略综合评估:")
print("=" * 70)
for metric, score in evaluation['scores'].items():
    status = "✓" if score['passed'] else "✗"
    print(f"{status} {metric}: {score['value']:.2%} (标准: {score['criterion']:.2%}, 评级: {score['grade']})")

print(f"\n综合评级: {evaluation['overall_grade']}")
print(f"建议: {evaluation['recommendation']}")
print("=" * 70)
```

### 12.3.3 稳定性测试

```python
class RobustnessTest:
    """稳健性测试"""

    @staticmethod
    def parameter_sensitivity(data, strategy_func, param_name, param_range):
        """
        参数敏感性测试

        参数:
        data: DataFrame, 数据
        strategy_func: function, 策略函数
        param_name: str, 参数名
        param_range: list, 参数取值范围

        返回:
        DataFrame: 各参数值下的表现
        """
        results = []

        for param_value in param_range:
            # 运行策略
            signals = strategy_func(data, **{param_name: param_value})

            # 简单计算收益(实际应该完整回测)
            returns = data['close'].pct_change()
            strategy_returns = (signals.shift(1) * returns).sum()

            results.append({
                param_name: param_value,
                'return': strategy_returns
            })

        return pd.DataFrame(results)

    @staticmethod
    def time_period_stability(data, strategy_func, period_length=60):
        """
        不同时间段稳定性测试

        参数:
        data: DataFrame, 数据
        strategy_func: function, 策略函数
        period_length: int, 每个时段的长度

        返回:
        DataFrame: 各时段表现
        """
        results = []
        num_periods = len(data) // period_length

        for i in range(num_periods):
            start = i * period_length
            end = start + period_length
            period_data = data.iloc[start:end]

            signals = strategy_func(period_data)
            returns = period_data['close'].pct_change()
            strategy_returns = (signals.shift(1) * returns).sum()

            results.append({
                'period': i + 1,
                'start_date': period_data.index[0],
                'end_date': period_data.index[-1],
                'return': strategy_returns
            })

        df = pd.DataFrame(results)

        # 计算统计量
        stats = {
            'mean_return': df['return'].mean(),
            'std_return': df['return'].std(),
            'positive_periods': (df['return'] > 0).sum(),
            'total_periods': len(df),
            'consistency': (df['return'] > 0).sum() / len(df)
        }

        return df, stats

# 示例
print("\n参数敏感性测试(不同均线周期):")
# 这里需要修改strategy函数以接受参数
def parameterized_strategy(data, short_window=5):
    signals = pd.Series(0, index=data.index)
    ma_short = data['close'].rolling(short_window).mean()
    ma_long = data['close'].rolling(20).mean()
    signals[ma_short > ma_long] = 1
    signals[ma_short < ma_long] = -1
    return signals

sensitivity = RobustnessTest.parameter_sensitivity(
    data, parameterized_strategy, 'short_window', range(3, 11)
)
print(sensitivity)

print("\n不同时间段稳定性测试:")
period_results, period_stats = RobustnessTest.time_period_stability(
    data, simple_strategy, period_length=50
)
print(f"平均收益: {period_stats['mean_return']:.2%}")
print(f"收益标准差: {period_stats['std_return']:.2%}")
print(f"正收益期数: {period_stats['positive_periods']}/{period_stats['total_periods']}")
print(f"一致性: {period_stats['consistency']:.2%}")
```

---

## 12.4 从回测到实盘 🚀

### 12.4.1 模拟交易 (Paper Trading)

**在真实市场环境中用虚拟资金测试策略。**

```python
class PaperTradingAccount:
    """模拟交易账户"""

    def __init__(self, initial_capital, commission_rate=0.0003):
        """
        参数:
        initial_capital: float, 初始资金
        commission_rate: float, 手续费率
        """
        self.initial_capital = initial_capital
        self.cash = initial_capital
        self.commission_rate = commission_rate
        self.positions = {}  # {symbol: {'shares': int, 'avg_price': float}}
        self.orders = []
        self.trades = []

    def place_order(self, symbol, shares, price, order_type='BUY'):
        """
        下单

        参数:
        symbol: str, 标的代码
        shares: int, 数量
        price: float, 价格
        order_type: str, 'BUY' 或 'SELL'

        返回:
        dict: 订单信息
        """
        order = {
            'symbol': symbol,
            'shares': shares,
            'price': price,
            'type': order_type,
            'time': pd.Timestamp.now(),
            'status': 'PENDING'
        }
        self.orders.append(order)
        return order

    def execute_order(self, order, actual_price=None):
        """
        执行订单

        参数:
        order: dict, 订单
        actual_price: float, 实际成交价(模拟滑点)

        返回:
        bool: 是否成功
        """
        if actual_price is None:
            actual_price = order['price']

        symbol = order['symbol']
        shares = order['shares']
        order_type = order['type']

        if order_type == 'BUY':
            cost = shares * actual_price
            commission = cost * self.commission_rate

            if self.cash >= cost + commission:
                self.cash -= (cost + commission)

                if symbol not in self.positions:
                    self.positions[symbol] = {'shares': 0, 'avg_price': 0}

                # 更新平均成本
                current_shares = self.positions[symbol]['shares']
                current_avg_price = self.positions[symbol]['avg_price']
                new_shares = current_shares + shares
                new_avg_price = (current_shares * current_avg_price + cost) / new_shares

                self.positions[symbol]['shares'] = new_shares
                self.positions[symbol]['avg_price'] = new_avg_price

                order['status'] = 'FILLED'
                self.trades.append({
                    'type': 'BUY',
                    'symbol': symbol,
                    'shares': shares,
                    'price': actual_price,
                    'commission': commission,
                    'time': pd.Timestamp.now()
                })
                return True
            else:
                order['status'] = 'REJECTED'
                return False

        elif order_type == 'SELL':
            if symbol in self.positions and self.positions[symbol]['shares'] >= shares:
                revenue = shares * actual_price
                commission = revenue * self.commission_rate
                self.cash += (revenue - commission)

                self.positions[symbol]['shares'] -= shares
                if self.positions[symbol]['shares'] == 0:
                    del self.positions[symbol]

                order['status'] = 'FILLED'
                self.trades.append({
                    'type': 'SELL',
                    'symbol': symbol,
                    'shares': shares,
                    'price': actual_price,
                    'commission': commission,
                    'time': pd.Timestamp.now()
                })
                return True
            else:
                order['status'] = 'REJECTED'
                return False

    def get_portfolio_value(self, market_prices):
        """
        获取组合总值

        参数:
        market_prices: dict, {symbol: current_price}

        返回:
        float: 组合总值
        """
        holdings_value = sum(
            pos['shares'] * market_prices.get(symbol, pos['avg_price'])
            for symbol, pos in self.positions.items()
        )
        return self.cash + holdings_value

    def get_account_summary(self, market_prices):
        """获取账户摘要"""
        total_value = self.get_portfolio_value(market_prices)
        return {
            'cash': self.cash,
            'holdings_value': total_value - self.cash,
            'total_value': total_value,
            'return': (total_value - self.initial_capital) / self.initial_capital,
            'positions': self.positions.copy(),
            'num_trades': len(self.trades)
        }

# 示例
account = PaperTradingAccount(initial_capital=100000)

# 下单买入
order1 = account.place_order('000001', shares=1000, price=10.0, order_type='BUY')
account.execute_order(order1, actual_price=10.05)  # 模拟滑点

# 查看账户
market_prices = {'000001': 10.50}
summary = account.get_account_summary(market_prices)
print("\n模拟交易账户摘要:")
for key, value in summary.items():
    if key != 'positions':
        print(f"{key}: {value}")
```

### 12.4.2 实盘前的检查清单

```python
class PreLiveTradingChecklist:
    """实盘前检查清单"""

    def __init__(self):
        self.checklist = {
            '策略验证': [
                '回测时间跨度≥3年',
                '包含多种市场环境(牛市/熊市/震荡)',
                '样本外验证通过',
                '参数稳健性测试通过',
                '交易成本已充分考虑',
            ],
            '风险管理': [
                '明确最大单笔损失限额',
                '明确最大回撤容忍度',
                '设置了止损机制',
                '仓位管理规则清晰',
                '建立了交易暂停机制',
            ],
            '技术准备': [
                '交易接口测试完成',
                '订单执行流程验证',
                '异常处理机制完善',
                '日志记录系统就绪',
                '监控告警系统部署',
            ],
            '资金准备': [
                '使用可承受损失的闲置资金',
                '预留至少6个月生活费',
                '不使用杠杆或严格控制杠杆',
                '从小资金开始(≤总资金10%)',
            ],
            '心理准备': [
                '理解策略的风险特征',
                '接受正常范围内的回撤',
                '不期望一夜暴富',
                '准备好长期坚持',
                '能够承受连续亏损的压力',
            ]
        }

    def display_checklist(self):
        """显示检查清单"""
        print("\n" + "=" * 70)
        print("实盘前检查清单")
        print("=" * 70)

        for category, items in self.checklist.items():
            print(f"\n【{category}】")
            for i, item in enumerate(items, 1):
                print(f"  {i}. [ ] {item}")

        print("\n" + "=" * 70)
        print("请确保所有项目都已完成检查并打勾!")
        print("=" * 70)

checklist = PreLiveTradingChecklist()
checklist.display_checklist()
```

### 12.4.3 实盘监控

```python
class LiveTradingMonitor:
    """实盘监控系统"""

    def __init__(self, strategy_name):
        """
        参数:
        strategy_name: str, 策略名称
        """
        self.strategy_name = strategy_name
        self.alerts = []

    def check_performance(self, current_results, expected_results):
        """
        检查实盘表现是否与预期一致

        参数:
        current_results: dict, 当前实盘结果
        expected_results: dict, 回测预期结果

        返回:
        list: 告警信息
        """
        alerts = []

        # 1. 收益率偏差
        return_diff = current_results['return'] - expected_results['return']
        if abs(return_diff) > 0.05:  # 偏差超过5%
            alerts.append({
                'level': 'WARNING',
                'metric': 'return',
                'message': f"收益率偏差较大: 实际{current_results['return']:.2%} "
                          f"vs 预期{expected_results['return']:.2%}"
            })

        # 2. 胜率偏差
        if 'win_rate' in current_results and 'win_rate' in expected_results:
            wr_diff = current_results['win_rate'] - expected_results['win_rate']
            if abs(wr_diff) > 0.1:  # 偏差超过10%
                alerts.append({
                    'level': 'WARNING',
                    'metric': 'win_rate',
                    'message': f"胜率偏差较大: 实际{current_results['win_rate']:.2%} "
                              f"vs 预期{expected_results['win_rate']:.2%}"
                })

        # 3. 回撤检查
        if current_results.get('max_drawdown', 0) < -0.15:
            alerts.append({
                'level': 'CRITICAL',
                'metric': 'max_drawdown',
                'message': f"回撤过大: {current_results['max_drawdown']:.2%}"
            })

        # 4. 交易频率
        if 'num_trades' in current_results and 'num_trades' in expected_results:
            trade_ratio = current_results['num_trades'] / max(expected_results['num_trades'], 1)
            if trade_ratio < 0.5 or trade_ratio > 2.0:
                alerts.append({
                    'level': 'WARNING',
                    'metric': 'num_trades',
                    'message': f"交易频率异常: 实际{current_results['num_trades']} "
                              f"vs 预期{expected_results['num_trades']}"
                })

        self.alerts.extend(alerts)
        return alerts

    def generate_daily_report(self, account_summary, trades_today):
        """
        生成日报

        参数:
        account_summary: dict, 账户摘要
        trades_today: list, 今日交易

        返回:
        str: 报告内容
        """
        report = f"""
{'=' * 60}
{self.strategy_name} - 每日监控报告
日期: {pd.Timestamp.now().strftime('%Y-%m-%d')}
{'=' * 60}

【账户概况】
总资产: {account_summary['total_value']:,.2f}元
现金: {account_summary['cash']:,.2f}元
持仓市值: {account_summary['holdings_value']:,.2f}元
累计收益率: {account_summary['return']:.2%}

【今日交易】
交易笔数: {len(trades_today)}
"""
        for trade in trades_today:
            report += f"  {trade['type']} {trade['symbol']} "
            report += f"{trade['shares']}股 @ {trade['price']:.2f}元\n"

        if self.alerts:
            report += "\n【告警信息】\n"
            for alert in self.alerts:
                report += f"  [{alert['level']}] {alert['metric']}: {alert['message']}\n"
        else:
            report += "\n【告警信息】\n  无告警\n"

        report += "=" * 60

        return report

# 示例
monitor = LiveTradingMonitor(strategy_name="双均线策略")

# 检查表现
current = {'return': 0.08, 'win_rate': 0.52, 'max_drawdown': -0.12, 'num_trades': 25}
expected = {'return': 0.10, 'win_rate': 0.55, 'max_drawdown': -0.15, 'num_trades': 30}

alerts = monitor.check_performance(current, expected)
if alerts:
    print("\n发现告警:")
    for alert in alerts:
        print(f"[{alert['level']}] {alert['message']}")

# 生成日报
account_summary = {
    'total_value': 108000,
    'cash': 50000,
    'holdings_value': 58000,
    'return': 0.08
}

trades_today = [
    {'type': 'BUY', 'symbol': '000001', 'shares': 500, 'price': 10.2},
    {'type': 'SELL', 'symbol': '000002', 'shares': 300, 'price': 15.8}
]

report = monitor.generate_daily_report(account_summary, trades_today)
print(report)
```

### 12.4.4 实盘常见问题

```python
class CommonLiveTradingIssues:
    """实盘常见问题及解决方案"""

    @staticmethod
    def get_issues_and_solutions():
        issues = {
            '滑点超出预期': {
                'description': '实际成交价与预期价差较大',
                'causes': [
                    '流动性不足',
                    '市价单使用不当',
                    '市场波动剧烈'
                ],
                'solutions': [
                    '使用限价单',
                    '分批建仓/平仓',
                    '避开开盘和收盘',
                    '选择流动性好的标的'
                ]
            },

            '策略失效': {
                'description': '实盘表现远不如回测',
                'causes': [
                    '过拟合',
                    '市场环境改变',
                    '回测陷阱未避免'
                ],
                'solutions': [
                    '停止交易,重新评估',
                    '检查策略逻辑',
                    '样本外验证',
                    '市场适应性调整'
                ]
            },

            '情绪化交易': {
                'description': '未按策略执行,情绪干扰',
                'causes': [
                    '连续亏损压力',
                    '贪婪追求更高收益',
                    '恐惧错过机会'
                ],
                'solutions': [
                    '完全自动化执行',
                    '设置强制休息期',
                    '降低仓位缓解压力',
                    '记录交易日志反思'
                ]
            },

            '技术故障': {
                'description': '系统崩溃、断网等',
                'causes': [
                    '代码bug',
                    '网络中断',
                    '服务器故障'
                ],
                'solutions': [
                    '建立备用系统',
                    '设置人工监控',
                    '紧急平仓预案',
                    '异常情况告警'
                ]
            },

            '资金管理失控': {
                'description': '亏损超过预期,资金大幅缩水',
                'causes': [
                    '未严格止损',
                    '仓位过重',
                    '连续加仓摊平'
                ],
                'solutions': [
                    '严格执行止损',
                    '遵守仓位限制',
                    '启动暂停机制',
                    '及时止损认错'
                ]
            }
        }

        return issues

    @staticmethod
    def display_issues():
        issues = CommonLiveTradingIssues.get_issues_and_solutions()

        print("\n" + "=" * 70)
        print("实盘常见问题及解决方案")
        print("=" * 70)

        for issue_name, details in issues.items():
            print(f"\n【{issue_name}】")
            print(f"问题描述: {details['description']}")

            print("\n可能原因:")
            for cause in details['causes']:
                print(f"  • {cause}")

            print("\n解决方案:")
            for solution in details['solutions']:
                print(f"  ✓ {solution}")

        print("\n" + "=" * 70)

CommonLiveTradingIssues.display_issues()
```

---

## 12.5 本章小结 📝

### 核心要点回顾

1. **回测是策略验证的基础**
   - 使用历史数据模拟交易
   - 评估策略的盈利能力和风险特征
   - 是实盘前的必经步骤

2. **警惕回测陷阱**
   - 前视偏差：不能使用未来信息
   - 幸存者偏差：要包含退市标的
   - 过度拟合：策略过于复杂容易失效
   - 交易成本：必须充分考虑
   - 流动性：要评估实际可成交性

3. **识别好策略的方法**
   - 样本外验证(训练集/测试集分离)
   - 滚动优化分析
   - 综合指标评估(夏普、回撤、胜率等)
   - 参数稳健性测试
   - 不同时期表现一致性

4. **从回测到实盘的过渡**
   - 先进行模拟交易(Paper Trading)
   - 完成实盘前检查清单
   - 从小资金开始
   - 建立监控告警系统
   - 准备好应对常见问题

### 回测与实盘的差异

| 方面 | 回测 | 实盘 |
|-----|------|------|
| **数据** | 完美的历史数据 | 实时数据可能有延迟或错误 |
| **执行** | 假设完美执行 | 存在滑点、部分成交等 |
| **成本** | 可以精确设定 | 实际成本可能波动 |
| **心理** | 无压力 | 真金白银的心理压力 |
| **优化** | 可以反复调试 | 每笔交易都是真实的 |
| **时间** | 快速完成 | 需要实时等待 |

### 实用建议 💡

1. **保持谦逊**
   - 回测再完美也不代表实盘一定成功
   - 市场在不断变化,没有永远有效的策略
   - 持续学习和适应是关键

2. **小步快跑**
   - 从小资金开始实盘
   - 逐步增加投入
   - 每次失败都是学习机会

3. **严格纪律**
   - 制定规则并严格执行
   - 不要因为短期波动改变策略
   - 定期评估,理性调整

4. **风险第一**
   - 永远把风险控制放在首位
   - 保护资金比追求收益更重要
   - 活下来才有机会盈利

---

**恭喜你完成知识篇！** 🎉

通过第十章到第十二章,你已经系统学习了量化策略的基础知识、风险管理方法,以及回测与实盘的完整流程。接下来,我们将进入**技术篇**,深入学习Python编程、数据分析、可视化等量化交易的必备技术技能。

**下一章预告** 👉

第十三章《Python编程基础》将带你从零开始学习Python,为后续的量化交易实践打下坚实的编程基础。即使你完全没有编程经验,也能通过系统学习掌握这门强大的工具！
